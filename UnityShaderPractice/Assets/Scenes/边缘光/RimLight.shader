...
Shader "Unlit/RimLight"
{
	Properties
	{
		...
		// 边缘光强度
		_RimPower("RimPower", Range(0.1, 3.0)) = 0.1
		// 边缘光Mask贴图
	...
	SubShader
	{
		...
		Pass
		{
			...
			sampler2D _MainTex;
			sampler2D _RimMask;
			// 用了TRANSFORM_TEX这个函数一定要定义_MainTex_ST
			float4 _MainTex_ST;
			fixed4 _RimColor;
			float _RimPower;
			float _RimSpeed;
			...
			float4 frag(v2f o) : SV_Target
			{
				...
				// 计算边缘光
				float rim = 1 - max(0, dot(viewDir, o.normal));
				fixed3 rimColor = _RimColor * pow(rim, 1 / _RimPower);
				// 通过Mask贴图采样
				fixed rimMask = tex2D(_RimMask, o.uv + float2(0, _Time.y * _RimSpeed)).r;

				return float4(rimColor * (rimMask), 1);
			}
			ENDCG
		}
		...
		Pass
		{
			...
			float4 frag(v2f o) : SV_Target
			{
				...
				if (dot(o.normal, lightDir) > 0.0)
				{
					...
					specular = _LightColor0.rgb * pow(max(0.0, dot(halfVector, o.normal)), _SpecShininess);
				}
